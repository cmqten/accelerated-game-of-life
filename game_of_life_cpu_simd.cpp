 ...
 *   to get the integer "vector" sum. The case where the sum of one cell will 
 *   contaminate the next cell because of the carry out will never occur because 
 *   the maximum is 8, which is well below 255. The result should be an n-byte 
 *   integer "vector" in which each byte is between 0 and 8.
